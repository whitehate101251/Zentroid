...
#define COMMON_CLEAN_MODE_KEY "CleanMode"
#define COMMON_CLEAN_MODE_DEF true

#define COMMON_DARK_MODE_KEY "DarkMode"
#define COMMON_DARK_MODE_DEF true

#define COMMON_RENDER_DRIVER_INDEX_KEY "RenderDriverIndex"
#define COMMON_RENDER_DRIVER_INDEX_DEF -1
...
const QString &Config::getConfigPath()
{
    if (s_configPath.isEmpty()) {
        s_configPath = QString::fromLocal8Bit(qgetenv("ZENTROID_CONFIG_PATH"));
        QFileInfo fileInfo(s_configPath);
        if (s_configPath.isEmpty() || !fileInfo.isDir()) {
            // default application dir
            // on macOS, when launching app from Finder, the default working directory is "/" instead of the executable's directory
            // Qt's working directory APIs depend on QCoreApplication initialization, so use macOS API to get the current directory
#ifdef Q_OS_OSX
            // get */Zentroid.app path
...
void Config::setUserBootConfig(const UserBootConfig &config)
{
    ...
    m_userData->setValue(COMMON_CLEAN_MODE_KEY, config.cleanMode);
    m_userData->setValue(COMMON_DARK_MODE_KEY, config.darkMode);
    m_userData->setValue(COMMON_RENDER_DRIVER_INDEX_KEY, config.renderDriverIndex);
    m_userData->endGroup();
    m_userData->sync();
}
...
UserBootConfig Config::getUserBootConfig()
{
    ...
    config.cleanMode = m_userData->value(COMMON_CLEAN_MODE_KEY, COMMON_CLEAN_MODE_DEF).toBool();
    config.darkMode = m_userData->value(COMMON_DARK_MODE_KEY, COMMON_DARK_MODE_DEF).toBool();
    config.renderDriverIndex = m_userData->value(COMMON_RENDER_DRIVER_INDEX_KEY, COMMON_RENDER_DRIVER_INDEX_DEF).toInt();
    m_userData->endGroup();
    return config;
}
...
void Config::saveIpHistory(const QString &ip)
{
    ...
    // remove existing duplicate IP (avoid duplicates)
    ipList.removeAll(ip);
    
    // prepend new IP to the front
    ipList.prepend(ip);
...
void Config::savePortHistory(const QString &port)
{
    ...
    // remove existing duplicate port (avoid duplicates)
    portList.removeAll(port);
    
    // prepend new port to the front
