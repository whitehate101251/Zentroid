...
void ToolForm::initStyle()
{
    ...
    IconHelper::Instance()->SetIcon(ui->keymapOverlayBtn, QChar(0xf11c), 15); // keyboard icon
}
...
void ToolForm::mouseDoubleClickEvent(QMouseEvent *event)
{
    Q_UNUSED(event)
    setCollapsed(!m_collapsed);
}

void ToolForm::setCollapsed(bool collapsed)
{
    m_collapsed = collapsed;
    // Show/hide all buttons inside the layout
    ...
    if (collapsed) {
        setFixedWidth(12);
        setToolTip("Double-click to expand toolbar");
    } else {
...
void ToolForm::on_keymapOverlayBtn_clicked()
{
    VideoForm *vf = dynamic_cast<VideoForm*>(parent());
    if (!vf) return;

    KeymapOverlay *overlay = vf->keymapOverlay();
    if (!overlay) return;

    if (!overlay->isVisible()) {
        // Let user pick which keymap to load
        QString keymapDir = getCanonicalKeymapDir();
        QString keymapPath = QFileDialog::getOpenFileName(
            this, tr("Select Keymap"), keymapDir,
            tr("Keymap Files (*.json)"));
        if (!keymapPath.isEmpty()) {
            vf->showKeymapOverlay(keymapPath);
        }
    } else {
        // Toggle between play/edit mode, or hide if already in play
        if (overlay->mode() == KeymapOverlay::PlayMode) {
            overlay->toggleMode(); // -> edit
        } else {
            overlay->setMode(KeymapOverlay::PlayMode);
            overlay->hide();
        }
    }
}