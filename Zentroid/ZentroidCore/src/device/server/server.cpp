...
#define SERVER_DEBUGGER_PORT "5005"
    ...
    // @ means locked, no @ means unlocked
    // value specifies direction, no value means original orientation
    ...
    args << "audio=false";
    // server default is -1, can be omitted
    if (-1 != m_params.scid) {
    ...
    // the parameters below use server defaults; minimize parameter passing as too many args cause Samsung phones to error: stack corruption detected (-fstack-protector)
    /*
    ...
    */
    ...
    // this adb command runs blocking; m_serverProcess will not exit
...
bool Server::readInfo(VideoSocket *videoSocket, QString &deviceName, QSize &size)
{
    ...
    // first 4 bytes are AVCodecID, currently only H264 is supported so we skip parsing it
...
void Server::onConnectTimer()
{
    ...
    // connecting too early when Android listening socket is not yet established causes readInfo to fail, so we use a retry strategy
    // retry every 100ms, up to MAX_CONNECT_COUNT times
    QString deviceName;
    QSize deviceSize;
    bool success = false;
    ...
    if (!videoSocket->waitForConnected(1000)) {
        // connecting to adb is fast, no retry on failure here
        ...
        if (SSS_NULL != m_serverStartStep) {
            ...
            case SSS_ENABLE_TUNNEL_REVERSE:
                if (qsc::AdbProcess::AER_SUCCESS_EXEC == processResult) {
                    ...
                    // some devices report "more than one device" error on reverse, an adb bug
