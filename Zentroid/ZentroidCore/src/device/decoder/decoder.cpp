...
bool Decoder::push(const AVPacket *packet)
{
...
#ifdef ZENTROID_LAVF_HAS_NEW_ENCODING_DECODING_API
    int ret = -1;
    ...
    if (!ret) {
        ...
        /*
        // m_conver converts YUV to RGB using CPU, which is too CPU-intensive; switched to OpenGL for YUV rendering
        // QImage::copy is also very memory-intensive; this approach is not considered
        if (!m_conver.isInit()) {
