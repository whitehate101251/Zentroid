...
{
    qInfo() << "[Keymap] Controller::updateScript called, script length:" << gameScript.length();

    // If we already have an InputConvertGame and the new script is non-empty,
    // hot-reload the keymap data without destroying the converter.
    // This preserves m_gameMap (activation state) and cursor grab across edits.
    if (!gameScript.isEmpty() && m_inputConvert) {
        InputConvertGame *existing = dynamic_cast<InputConvertGame*>(m_inputConvert.data());
        if (existing) {
            existing->loadKeyMap(gameScript);
            qInfo() << "[Keymap] Hot-reloaded keymap into existing InputConvertGame (activation preserved)";
            return;
        }
    }

    // Otherwise, destroy and recreate (type change or first load)
    if (m_inputConvert) {
    ...
    if (!gameScript.isEmpty()) {
        ...
        qInfo() << "[Keymap] Created new InputConvertGame";
    } else {
        m_inputConvert = new InputConvertNormal(this);
        qInfo() << "[Keymap] Created InputConvertNormal (no script)";
    }
