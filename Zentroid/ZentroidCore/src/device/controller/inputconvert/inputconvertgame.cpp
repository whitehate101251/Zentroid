...
void InputConvertGame::mouseEvent(const QMouseEvent *from, const QSize &frameSize, const QSize &showSize)
{
    ...
    if (!m_needBackMouseMove && m_gameMap && !m_suspended) {
...
{
    // ── Switch key (Ctrl+P toggle) ──
    if (m_keyMap.isSwitchOnKeyboard() && m_keyMap.getSwitchKey() == from->key()) {
        // Check modifier match (e.g. Ctrl+P requires Ctrl to be held)
        Qt::KeyboardModifiers requiredMods = m_keyMap.getSwitchModifiers();
        // Mask out KeypadModifier and GroupSwitchModifier which are irrelevant
        Qt::KeyboardModifiers actualMods = from->modifiers() & (Qt::ControlModifier | Qt::AltModifier | Qt::ShiftModifier | Qt::MetaModifier);
        if (actualMods == requiredMods) {
            qInfo() << "[Keymap] Switch key pressed (keyboard):" << from->key() << "modifiers:" << actualMods;
    ...
    // ── Suspend key (X hold-to-disable) ──
    if (m_keyMap.hasSuspendKey() && from->key() == m_keyMap.getSuspendKey() && m_gameMap) {
        if (from->type() == QEvent::KeyPress && !from->isAutoRepeat()) {
            if (!m_suspended) {
                m_suspended = true;
                qInfo() << "[Keymap] Suspend key HELD — game mode temporarily disabled";
                // Release cursor while suspended
                if (m_keyMap.isValidMouseMoveMap()) {
            ...
            if (m_suspended) {
                m_suspended = false;
                qInfo() << "[Keymap] Suspend key RELEASED — game mode re-enabled";
                // Re-grab cursor
                if (m_keyMap.isValidMouseMoveMap()) {
    ...
    if (m_gameMap && !m_suspended) {
        ...
        // If this key is bound in the custom keymap, block it from reaching Android
        // so that games with built-in keyboard support (e.g. PUBG) don't pick it up.
        // Only the custom keymap (activated with the switch key) should handle these.
        if (node.type != KeyMap::KMT_INVALID) {
            return;  // key is mapped — swallow it silently
        }
...
void InputConvertGame::loadKeyMap(const QString &json)
{
    ...
    qInfo() << "[Keymap] InputConvertGame loaded keymap, switchOnKeyboard:"
            << m_keyMap.isSwitchOnKeyboard()
            << "switchKey:" << m_keyMap.getSwitchKey()
            << "hasMouseMoveMap:" << m_keyMap.isValidMouseMoveMap();

    // If game mode is already active (e.g. hot-reload after F12 edit),
    // re-arm cursor grab and mouse-look so the user doesn't have to
    // toggle X twice to recover.
    if (m_gameMap && m_keyMap.isValidMouseMoveMap()) {
        qInfo() << "[Keymap] Re-arming cursor grab after hot-reload (game mode was active)";
        emit grabCursor(true);
        hideMouseCursor(true);
    }
}
...
{
    m_gameMap = !m_gameMap;
    m_suspended = false;  // reset suspend state on toggle
    qInfo() << QString("[Keymap] switchGameMap → mode: %1").arg(m_gameMap ? "GAME (custom)" : "NORMAL");

    if (m_gameMap) {
        // Enabling game mode — only grab cursor if mouseMoveMap is valid
        if (m_keyMap.isValidMouseMoveMap()) {
            qInfo() << "[Keymap] Emitting grabCursor(true)";
            emit grabCursor(true);
            hideMouseCursor(true);
        } else {
            qInfo() << "[Keymap] No valid mouseMoveMap — cursor grab skipped (game mode still active for key bindings)";
        }
    } else {
        // Disabling game mode — ALWAYS release cursor and restore it,
        // even if mouseMoveMap is invalid, to avoid stuck cursor state.
        qInfo() << "[Keymap] Emitting grabCursor(false)";
        emit grabCursor(false);
        hideMouseCursor(false);
        stopMouseMoveTimer();
        mouseMoveStopTouch();
