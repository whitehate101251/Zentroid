...
{
    // Clear previous state so hot-reloads don't accumulate duplicate nodes
    m_keyMapNodes.clear();
    m_idxMouseMove = -1;
    m_idxSteerWheel = -1;
    m_rmapKey.clear();
    ...
    {
        // Parse switchKey — supports modifiers like "Ctrl+P", "Alt+F10", etc.
        QString switchKeyStr = getItemString(rootObj, "switchKey");
        m_switchModifiers = Qt::NoModifier;
        QString baseKeyStr = switchKeyStr;

        // Extract modifier prefixes (Ctrl+, Alt+, Shift+, Meta+)
        while (true) {
            if (baseKeyStr.startsWith("Ctrl+", Qt::CaseInsensitive)) {
                m_switchModifiers |= Qt::ControlModifier;
                baseKeyStr = baseKeyStr.mid(5);
            } else if (baseKeyStr.startsWith("Alt+", Qt::CaseInsensitive)) {
                m_switchModifiers |= Qt::AltModifier;
                baseKeyStr = baseKeyStr.mid(4);
            } else if (baseKeyStr.startsWith("Shift+", Qt::CaseInsensitive)) {
                m_switchModifiers |= Qt::ShiftModifier;
                baseKeyStr = baseKeyStr.mid(6);
            } else if (baseKeyStr.startsWith("Meta+", Qt::CaseInsensitive)) {
                m_switchModifiers |= Qt::MetaModifier;
                baseKeyStr = baseKeyStr.mid(5);
            } else {
                break;
            }
        ...
        // If the base key doesn't start with "Key_", add it
        if (!baseKeyStr.startsWith("Key_") && !baseKeyStr.startsWith("LeftButton") && !baseKeyStr.startsWith("RightButton")) {
            baseKeyStr = "Key_" + baseKeyStr;
        }

        int key = m_metaEnumKey.keyToValue(baseKeyStr.toStdString().c_str());
        int btn = m_metaEnumMouseButtons.keyToValue(baseKeyStr.toStdString().c_str());
        if (key == -1 && btn == -1) {
            errorString = QString("json error: switchKey invalid: %1").arg(switchKeyStr);
            goto parseError;
        } else if (key != -1) {
            m_switchKey.type = AT_KEY;
            m_switchKey.key = key;
        } else {
            m_switchKey.type = AT_MOUSE;
            m_switchKey.key = btn;
        }

        qInfo() << "[Keymap] Parsed switchKey:" << switchKeyStr
                << "→ baseKey:" << baseKeyStr << "(" << m_switchKey.key << ")"
                << "modifiers:" << m_switchModifiers;
    }

    // suspendKey (optional) — hold to temporarily disable game mode
    m_suspendKey = { AT_INVALID, -1 };
    if (checkItemString(rootObj, "suspendKey")) {
        QPair<ActionType, int> suspKey = getItemKey(rootObj, "suspendKey");
        if (suspKey.first != AT_INVALID) {
            m_suspendKey.type = suspKey.first;
            m_suspendKey.key = suspKey.second;
            qInfo() << "[Keymap] Parsed suspendKey:" << m_suspendKey.key;
        }
    }
...
Qt::KeyboardModifiers KeyMap::getSwitchModifiers()
{
    return m_switchModifiers;
}

bool KeyMap::hasSuspendKey()
{
    return m_suspendKey.type != AT_INVALID;
}

int KeyMap::getSuspendKey()
{
    return m_suspendKey.key;
}
