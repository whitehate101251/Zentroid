...
const QString &AdbProcessImpl::getAdbPath()
{
    if (s_adbPath.isEmpty()) {
        ...
        potentialPaths << QString::fromLocal8Bit(qgetenv("ZENTROID_ADB_PATH")) << g_adbPath
#ifdef Q_OS_WIN32
        ...
        if (s_adbPath.isEmpty()) {
            // if none of the paths are valid, warn the user
            qWarning() << "ADB path not found";
        } else {
...
void AdbProcessImpl::initSignals()
{
    ...
    connect(this, static_cast<void (QProcess::*)(int, QProcess::ExitStatus)>(&QProcess::finished), this, [this](int exitCode, QProcess::ExitStatus exitStatus) {
        if (NormalExit == exitStatus && 0 == exitCode) {
            ...
            //P7C0218510000537        unauthorized - phone shows debug authorization popup, must allow debugging
