...
int main(int argc, char *argv[])
{
    // set env
#ifdef Q_OS_WIN32
    qputenv("ZENTROID_ADB_PATH", "../../../Zentroid/ZentroidCore/src/third_party/adb/win/adb.exe");
    qputenv("ZENTROID_SERVER_PATH", "../../../Zentroid/ZentroidCore/src/third_party/scrcpy-server");
    qputenv("ZENTROID_KEYMAP_PATH", "../../../keymap");
    qputenv("ZENTROID_CONFIG_PATH", "../../../config");
#endif

#ifdef Q_OS_OSX
    qputenv("ZENTROID_ADB_PATH", "../../../../../../Zentroid/ZentroidCore/src/third_party/adb/mac/adb");
    qputenv("ZENTROID_SERVER_PATH", "../../../../../../Zentroid/ZentroidCore/src/third_party/scrcpy-server");
    qputenv("ZENTROID_KEYMAP_PATH", "../../../../../../keymap");
    qputenv("ZENTROID_CONFIG_PATH", "../../../../../../config");
#endif
    ...
    if (qgetenv("ZENTROID_ADB_PATH").isEmpty()) {
        qputenv("ZENTROID_ADB_PATH", "../../../Zentroid/ZentroidCore/src/third_party/adb/linux/adb");
    }
    if (qgetenv("ZENTROID_SERVER_PATH").isEmpty()) {
        qputenv("ZENTROID_SERVER_PATH", "../../../Zentroid/ZentroidCore/src/third_party/scrcpy-server");
    }
    if (qgetenv("ZENTROID_KEYMAP_PATH").isEmpty()) {
        qputenv("ZENTROID_KEYMAP_PATH", "../../../keymap");
    }
    if (qgetenv("ZENTROID_CONFIG_PATH").isEmpty()) {
        qputenv("ZENTROID_CONFIG_PATH", "../../../config");
    }
#endif
    ...
    // Determine render driver: user setting takes priority, fallback to config.ini
    int opengl = Config::getInstance().getUserBootConfig().renderDriverIndex;
    if (opengl == -1) {
        // User hasn't overridden — use config.ini value
        opengl = Config::getInstance().getDesktopOpenGL();
    }
    if (0 == opengl) {
    ...
    // Disable VSync lock so rendering is not capped at 60fps.
    // This allows the display to render at the monitor's native refresh rate
    // (e.g. 90Hz, 120Hz, 144Hz, 165Hz, etc.)
    ...
    */
    ...
    // on Windows, version and app name are set via qmake VERSION variable or .rc file, accessible here
    ...
    qInfo() << QObject::tr("This software is completely open source and free. Use it at your own risk. You can download it at the "
            "following address:");
    qInfo() << QString("Zentroid %1 <https://github.com/whitehate101251/Zentroid>").arg(QCoreApplication::applicationVersion());
