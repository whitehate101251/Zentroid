...
appdir_path="$appimage_output_path/Zentroid.AppDir"
app_name="Zentroid"
app_version=$(cat Zentroid/appversion 2>/dev/null || echo "0.0.0")
...
adb_source="$project_root/Zentroid/ZentroidCore/src/third_party/adb/linux/adb"
server_source="$project_root/Zentroid/ZentroidCore/src/third_party/scrcpy-server"
mkdir -p "$appdir_path/usr/lib/zentroid"

if [ -f "$adb_source" ]; then
    cp "$adb_source" "$appdir_path/usr/lib/zentroid/adb"
    chmod +x "$appdir_path/usr/lib/zentroid/adb"
    # Create symlink for sndcpy.sh compatibility
    if [ ! -f "$appdir_path/usr/bin/adb" ]; then
        ln -sf "../lib/zentroid/adb" "$appdir_path/usr/bin/adb"
    fi
fi

if [ -f "$server_source" ]; then
    cp "$server_source" "$appdir_path/usr/lib/zentroid/scrcpy-server"
    chmod +x "$appdir_path/usr/lib/zentroid/scrcpy-server"
fi
...
if [ -f "$project_root/Zentroid/res/Zentroid.png" ]; then
    icon_source="$project_root/Zentroid/res/Zentroid.png"
elif [ -f "$project_root/Zentroid/res/image/tray/logo.png" ]; then
    icon_source="$project_root/Zentroid/res/image/tray/logo.png"
elif [ -f "$project_root/Zentroid/res/Zentroid.ico" ]; then
    icon_source="$project_root/Zentroid/res/Zentroid.ico"
fi
...
app_id="com.github.barry-ran.Zentroid"
...
<component type="desktop-application">
  ...
  <description>
    <p>Zentroid supports displaying and controlling Android devices via USB or over network. It does NOT require root privileges.</p>
  </description>
  <provides>
    <binary>$app_name</binary>
  </provides>
  <launchable type="desktop-id">$app_name.desktop</launchable>
  <url type="homepage">https://github.com/barry-ran/Zentroid</url>
...
export ZENTROID_ADB_PATH="$HERE/usr/lib/zentroid/adb"
export ZENTROID_SERVER_PATH="$HERE/usr/lib/zentroid/scrcpy-server"
export ZENTROID_KEYMAP_PATH="$HERE/usr/share/keymap"
export ZENTROID_CONFIG_PATH="$HERE/usr/share/config"
exec "$HERE/usr/bin/Zentroid" "$@"
APPRUN_EOF
else
    cat > "$appdir_path/AppRun" << 'APPRUN_EOF'
#!/bin/bash
...
export QML_SOURCES_PATHS="$project_root/Zentroid"
